// scripts/apply-batch1-landings.cjs
//
// Creates Batch 1 of the 20-landing project (multilingual-website,
// catalog-website, online-booking) from drafts/_batch1-docs.json, built by
// assemble-batch1-landings.cjs. Each landing's 4 docs (EN/PL/RU/i18n) are
// the atomic unit: if one landing fails, its own docs are rolled back and
// processing stops there, but any already-completed landings in this batch
// are left in place (they are independently valid).
//
// Usage: node scripts/apply-batch1-landings.cjs --apply
...
const allDocs = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../drafts/_batch1-docs.json"), "utf8")
);
...
async function main() {
  ...
  console.log("\n=== VERIFY ===");
  const verifyIds = names
    .filter((n) => results[n] === "created")
    .flatMap((name) => {
      const { docs, metaDoc } = allDocs[name];
      return [docs.en._id, docs.pl._id, docs.ru._id, metaDoc._id];
    });
