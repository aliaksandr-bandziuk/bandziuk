// scripts/apply-beauty-salon-cost-blog.cjs
//
// Creates the blog-beauty-salon-website-cost post (EN -> PL -> RU -> i18n
// metadata) from drafts/_beauty-salon-cost-docs.json, built by
// assemble-beauty-salon-cost-blog.cjs. Rolls back all docs created so far
// if any step fails.
//
// Usage: node scripts/apply-beauty-salon-cost-blog.cjs --apply
...
const { docs, metaDoc } = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../drafts/_beauty-salon-cost-docs.json"), "utf8")
);
...
async function main() {
  ...
  console.log("=== CREATING blog-beauty-salon-website-cost (EN -> PL -> RU -> i18n) ===");
  const createdIds = [];
  try {
