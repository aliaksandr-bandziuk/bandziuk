...
// Pass 4: this batch's special case — add a bidirectional backlink from the pre-existing
// published article blog-found-via-chatgpt to the new blog-chatgpt-recommendations, per the
// master prompt's explicit "link them to each other in both directions" instruction.
const fs = require("fs");
...
const allDocs = JSON.parse(fs.readFileSync(path.join(DRAFTS, "_blog-batch4-docs.json"), "utf8"));
...
const BACKFILL_PLAN = {
  "blog-how-to-choose-developer": ["blog-website-build-timeline"],
  "blog-how-to-choose-developer.pl": ["blog-website-build-timeline.pl"],
  "blog-how-to-choose-developer.ru": ["blog-website-build-timeline.ru"],
  "blog-freelancer-vs-agency": ["blog-website-build-timeline"],
  "blog-freelancer-vs-agency.pl": ["blog-website-build-timeline.pl"],
  "blog-freelancer-vs-agency.ru": ["blog-website-build-timeline.ru"],
  "blog-not-showing-in-google": ["blog-chatgpt-recommendations"],
  "blog-not-showing-in-google.pl": ["blog-chatgpt-recommendations.pl"],
  "blog-not-showing-in-google.ru": ["blog-chatgpt-recommendations.ru"],
};

// Special bidirectional backlink: the pre-existing published ChatGPT-discovery article should
// point at the new implementation/measurement article, not just the reverse.
const BACKLINK_PLAN = {
  "blog-found-via-chatgpt": ["blog-chatgpt-recommendations"],
  "blog-found-via-chatgpt.pl": ["blog-chatgpt-recommendations.pl"],
  "blog-found-via-chatgpt.ru": ["blog-chatgpt-recommendations.ru"],
};
...
async function main() {
  ...
  console.log("\n=== BACKLINK PLAN (Pass 4, pre-existing published article) ===");
  for (const [docId, refs] of Object.entries(BACKLINK_PLAN)) {
    console.log(`  ${docId}: += ${refs.join(", ")}`);
  }
  ...
  if (succeeded.length === plan.length) {
    ...
    console.log("\n=== PASS 4: BACKLINK pre-existing published article ===");
    const backlinkIds = Object.keys(BACKLINK_PLAN);
    const backlinkDocs = await client.fetch(`*[_id in $ids]{ _id, relatedArticles }`, { ids: backlinkIds });
    const backlinkMap = Object.fromEntries(backlinkDocs.map((d) => [d._id, d]));
    for (const [docId, refsToAdd] of Object.entries(BACKLINK_PLAN)) {
      const doc = backlinkMap[docId];
      if (!doc) { console.log(`SKIP ${docId} — doc not found`); continue; }
      const cur = doc.relatedArticles || [];
      const already = new Set(cur.map((r) => r._ref));
      const newRefs = refsToAdd.filter((r) => !already.has(r));
      ...
      const newArr = [...cur, ...newRefs.map((r) => ({ _key: require("crypto").randomBytes(6).toString("hex"), _type: "reference", _ref: r }))];
      await client.patch(docId).set({ relatedArticles: newArr }).commit();
      console.log(`Backlinked ${docId} += ${newRefs.join(", ")}`);
    }
  } else {
    console.log("\nSkipping Pass 2/3/4 — not all articles in this batch were created successfully.");
  }
  ...
  console.log("(This is the final batch — this list should be empty.)");
