// scripts/apply-builder-vs-custom-blog.cjs
//
// Creates the blog-builder-vs-custom post (EN -> PL -> RU -> i18n metadata)
// from drafts/_builder-vs-custom-docs.json, built by
// assemble-builder-vs-custom-blog.cjs. Rolls back all docs created so far
// if any step fails.
//
// Usage: node scripts/apply-builder-vs-custom-blog.cjs --apply
...
const { docs, metaDoc } = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../drafts/_builder-vs-custom-docs.json"), "utf8")
);
...
async function main() {
  ...
  console.log("=== CREATING blog-builder-vs-custom (EN -> PL -> RU -> i18n) ===");
  const createdIds = [];
  try {
