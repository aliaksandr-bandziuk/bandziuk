// scripts/apply-dental-clinic-landing.cjs
//
// Creates the singlepage-dental-clinic-website landing (EN -> PL -> RU ->
// i18n metadata) from drafts/_dental-clinic-docs.json, built by
// assemble-dental-clinic-landing.cjs. Rolls back all docs created so far if
// any step fails.
//
// Usage: node scripts/apply-dental-clinic-landing.cjs --apply
...
const { docs, metaDoc } = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../drafts/_dental-clinic-docs.json"), "utf8")
);
...
async function main() {
  ...
  console.log("=== CREATING singlepage-dental-clinic-website (EN -> PL -> RU -> i18n) ===");
  const createdIds = [];
  try {
