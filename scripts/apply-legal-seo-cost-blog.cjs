// scripts/apply-legal-seo-cost-blog.cjs
//
// Creates the blog-legal-seo-cost post (EN -> PL -> RU -> i18n metadata)
// from drafts/_legal-seo-cost-docs.json, built by assemble-legal-seo-cost-blog.cjs.
// Rolls back all docs created so far if any step fails.
//
// Usage: node scripts/apply-legal-seo-cost-blog.cjs --apply
...
const { docs, metaDoc } = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../drafts/_legal-seo-cost-docs.json"), "utf8")
);
...
async function main() {
  ...
  console.log("=== CREATING blog-legal-seo-cost (EN -> PL -> RU -> i18n) ===");
  const createdIds = [];
  try {
