// scripts/apply-realestate-checklist-blog.cjs
//
// Creates the blog-real-estate-agency-checklist post (EN -> PL -> RU -> i18n
// metadata) from drafts/_realestate-checklist-docs.json, built by
// assemble-realestate-checklist-blog.cjs. Rolls back all docs created so far
// if any step fails.
//
// Usage: node scripts/apply-realestate-checklist-blog.cjs --apply
...
const { docs, metaDoc } = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../drafts/_realestate-checklist-docs.json"), "utf8")
);
...
async function main() {
  ...
  console.log("=== CREATING blog-real-estate-agency-checklist (EN -> PL -> RU -> i18n) ===");
  const createdIds = [];
  try {
