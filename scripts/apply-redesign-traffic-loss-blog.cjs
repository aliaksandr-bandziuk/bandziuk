// scripts/apply-redesign-traffic-loss-blog.cjs
//
// Creates the blog-redesign-traffic-loss post (EN -> PL -> RU -> i18n
// metadata) from drafts/_redesign-traffic-loss-docs.json, built by
// assemble-redesign-traffic-loss-blog.cjs. Rolls back all docs created so
// far if any step fails.
//
// Usage: node scripts/apply-redesign-traffic-loss-blog.cjs --apply
...
const { docs, metaDoc } = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../drafts/_redesign-traffic-loss-docs.json"), "utf8")
);
...
async function main() {
  ...
  console.log("=== CREATING blog-redesign-traffic-loss (EN -> PL -> RU -> i18n) ===");
  const createdIds = [];
  try {
