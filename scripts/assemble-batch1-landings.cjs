// scripts/assemble-batch1-landings.cjs
//
// Assembles the full three-locale document set for Batch 1 landings
// (multilingual-website, catalog-website, online-booking) from
// drafts/_batch1-parsed.json, and writes the complete resolved structure to
// drafts/_batch1-docs.json for review. Does NOT write to Sanity.
//
// Block sequence: benefitsBlock -> gridBlock -> textContent -> stepsBlock ->
// faqBlock -> landingCtaBlock. No relatedServicesBlock / interlinking in
// this pass -- Phase 2 handles [RELATED] after all 20 exist.
//
// allowIntroBlock: true is set (lesson from the dental-clinic landing --
// hero requires this flag). No previewImage is set (no cover image
// provided), so hero still won't visually render until one is uploaded in
// Studio -- same precedent as every other page this project.
...
const parsed = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../drafts/_batch1-parsed.json"), "utf8")
);

// Source of truth: the ID/slug table from the master prompt.
const LANDINGS = [
  {
    name: "multilingual-website",
    baseId: "singlepage-multilingual-website",
    slug: { en: "multilingual-website-development", pl: "tworzenie-stron-wielojezycznych", ru: "razrabotka-multiyazychnogo-saita" },
  },
  {
    name: "catalog-website",
    baseId: "singlepage-catalog-website",
    slug: { en: "catalog-website-with-filters", pl: "strona-katalogowa-z-filtrami", ru: "sait-katalog-s-filtrami" },
  },
  {
    name: "online-booking",
    baseId: "singlepage-online-booking",
    slug: { en: "website-with-online-booking", pl: "strona-z-rezerwacja-online", ru: "sait-s-onlain-zapisyu" },
  },
];
...
function buildBenefitsBlock(r, lang) {
  return {
    _key: key(),
    _type: "benefitsBlock",
    title: r.PAIN[lang].title,
    benefits: r.PAIN[lang].items.map((it) => ({ _key: key(), title: it.title, description: it.description })),
  };
}
...
function buildGridBlock(r, lang) {
  return {
    _key: key(),
    _type: "gridBlock",
    title: r.FEATURES[lang].title,
    items: r.FEATURES[lang].items.map((it) => ({ _key: key(), title: it.title, description: it.description })),
  };
}
...
function buildStepsBlock(r, lang) {
  return {
    _key: key(),
    _type: "stepsBlock",
    title: r.STEPS[lang].title,
    steps: r.STEPS[lang].items.map((it, i) => ({ _key: key(), stepNumber: i + 1, title: it.title, description: it.description })),
  };
}
...
function buildFaqBlock(r, lang) {
  return {
    _key: key(),
    _type: "faqBlock",
    title: r.FAQ[lang].title,
    faq: {
      _type: "accordionBlock",
      items: r.FAQ[lang].items.map((it) => ({
        _key: key(),
        question: it.title,
        answer: [{ _key: key(), _type: "block", children: [{ _key: key(), _type: "span", marks: [], text: it.description }], markDefs: [], style: "normal" }],
      })),
    },
  };
}
...
function buildDoc(landing, lang) {
  ...
  const contentBlocks = [
    buildBenefitsBlock(r, lang),
    buildGridBlock(r, lang),
    buildTextContent(r, lang),
    buildStepsBlock(r, lang),
    buildFaqBlock(r, lang),
    buildLandingCtaBlock(r, lang),
  ];
  return {
    _id: docIdFor(landing.baseId, lang),
    _type: "singlepage",
    language: lang,
    pageType: "page",
    allowIntroBlock: true,
    title: r.HERO[lang].headline,
    excerpt: r.HERO[lang].subheadline,
...
for (const landing of LANDINGS) {
  ...
  const metaDoc = {
    _id: `${landing.baseId}.i18n`,
    _type: "translation.metadata",
    documentId: landing.baseId,
    translations: [
...
fs.writeFileSync(
  path.resolve(__dirname, "../drafts/_batch1-docs.json"),
...
if (errors.length) {
  ...
  console.log("OK — all noindex/slug/block-order checks passed for all 3 landings x 3 locales.");
}
...
console.log("\nFull resolved docs written to drafts/_batch1-docs.json");
