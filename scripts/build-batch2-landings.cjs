// scripts/build-batch2-landings.cjs
// Batch 2: veterinary-clinic, accounting-firm. See build-batch1-landings.cjs for notes.
const fs = require("fs");
const path = require("path");
const { parseTaggedFile } = require("./lib/landing-parser.cjs");

const LANDINGS = [
  { name: "veterinary-clinic", file: "landing-veterinary-clinic-tagged-content.md" },
  { name: "accounting-firm", file: "landing-accounting-firm-tagged-content.md" },
];
...
fs.writeFileSync(path.resolve(__dirname, "../drafts/_batch2-parsed.json"), JSON.stringify(result, null, 2));
...
console.log("\nParsed JSON written to drafts/_batch2-parsed.json");
