// scripts/build-batch4-landings.cjs
// Batch 4: fitness-studio, cleaning-company, restaurant. See build-batch1-landings.cjs for notes.
const fs = require("fs");
const path = require("path");
const { parseTaggedFile } = require("./lib/landing-parser.cjs");

const LANDINGS = [
  { name: "fitness-studio", file: "landing-fitness-studio-tagged-content.md" },
  { name: "cleaning-company", file: "landing-cleaning-company-tagged-content.md" },
  { name: "restaurant", file: "landing-restaurant-tagged-content.md" },
];
...
fs.writeFileSync(path.resolve(__dirname, "../drafts/_batch4-parsed.json"), JSON.stringify(result, null, 2));
...
console.log("\nParsed JSON written to drafts/_batch4-parsed.json");
