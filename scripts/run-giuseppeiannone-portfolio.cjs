// scripts/run-giuseppeiannone-portfolio.cjs
//
// Builds the giuseppeiannone.it portfolio case study in Sanity: three locale
// documents (en / pl / ru) plus the translation.metadata that ties them
// together, in a single transaction.
//
// Inputs
//   drafts/portfolio-giuseppeiannone-{EN,PL,RU}.md   the case text, section-tagged
//   drafts/screenshots-giuseppeiannone/assets.json   image asset ids, written by
//                                                  upload-giuseppeiannone-screenshots.cjs
//
// Run the screenshot uploader first — this script does not upload images.
//
// Usage:
//   node scripts/run-giuseppeiannone-portfolio.cjs --dry-run   parse, resolve refs, print, write nothing
//   node scripts/run-giuseppeiannone-portfolio.cjs             create the documents
...
const SLIDER_FILES = [
  ...
  "6-pillar-page.jpg",
  "7-blog-article.jpg",
  "8-english-version.jpg",
];
// Slider file -> numbered entry in the [SCREENSHOTS] block of the drafts.
const SLIDER_ENTRY = {
  "2-ti-riconosci.jpg": 2,
  "3-il-percorso.jpg": 3,
  "4-map.jpg": 4,
  "5-video-light.jpg": 5,
  "6-pillar-page.jpg": 6,
  "7-blog-article.jpg": 7,
  "8-english-version.jpg": 8,
};
...
const TECHNOLOGIES = [
  ...
  // Concept entry: the title differs per locale.
...
function readDraft(lang) {
  const p = path.join(DRAFTS, `portfolio-giuseppeiannone-${lang.toUpperCase()}.md`);
...
function buildDoc({ lang, docId, raw, manifest, techIds }) {
  ...
  const mainContent = parseMainContent(raw).map((sec) => {
    ...
    return {
      ...
      content: [
        ...
        // A paragraph starting with "> " is the client's review, quoted verbatim;
        // it goes in as a blockquote and the marker is stripped.
        ...paragraphs.map((t) =>
          t.startsWith("> ")
            ? ptBlock("blockquote", t.slice(2).trim(), { linkMap })
            : ptBlock("normal", t, { linkMap })
        ),
      ],
    };
  });
  ...
  return {
    ...
    keyFeatures: {
      ...
      website: {
        type: "link",
        linkLabel: "giuseppeiannone.it",
        linkDestination: "https://www.giuseppeiannone.it/",
      },
    },
...
async function main() {
  if (!fs.existsSync(MANIFEST)) {
    throw new Error(
      `No ${MANIFEST}. Run: node scripts/upload-giuseppeiannone-screenshots.cjs`
    );
  }
