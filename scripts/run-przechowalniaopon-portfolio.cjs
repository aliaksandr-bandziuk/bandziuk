// scripts/run-przechowalniaopon-portfolio.cjs
//
// Builds the przechowalniaopon.pl portfolio case study in Sanity: three locale
// documents (en / pl / ru) plus the translation.metadata that ties them
// together, in a single transaction.
//
// Inputs
//   drafts/portfolio-przechowalniaopon-{EN,PL,RU}.md   the case text, section-tagged
//   drafts/screenshots-przechowalniaopon/assets.json   image asset ids, written by
//                                                  upload-przechowalniaopon-screenshots.cjs
//
// Run the screenshot uploader first — this script does not upload images.
//
// Usage:
//   node scripts/run-przechowalniaopon-portfolio.cjs --dry-run   parse, resolve refs, print, write nothing
//   node scripts/run-przechowalniaopon-portfolio.cjs             create the documents
...
const SLIDER_FILES = [
  ...
  "6-faq.png",
  "7-contact-map.png",
  "8-english-version.png",
  "9-russian-version.png",
  "10-pagespeed.png",
];
// Slider file -> numbered entry in the [SCREENSHOTS] block of the drafts.
const SLIDER_ENTRY = {
  "2-address-hours.png": 2,
  "3-why-storage.png": 3,
  "4-pricing-table.png": 4,
  "5-enquiry-form.png": 5,
  "6-faq.png": 6,
  "7-contact-map.png": 7,
  "8-english-version.png": 8,
  "9-russian-version.png": 9,
  "10-pagespeed.png": 10,
};
...
const TECHNOLOGIES = [
  ...
  // Concept entries carry their own title per locale.
  { canonical: "Web Accessibility", aliases: ["Web Accessibility", "Web Dostępność", "Web Доступность"] },
  { canonical: "Hosting & Deployment", aliases: ["Hosting & Deployment", "Hosting i wdrażanie", "Хостинг и деплой"] },
];
...
function readDraft(lang) {
  const p = path.join(DRAFTS, `portfolio-przechowalniaopon-${lang.toUpperCase()}.md`);
...
function buildDoc({ lang, docId, raw, manifest, techIds }) {
  ...
  return {
    ...
    keyFeatures: {
      ...
      website: {
        type: "link",
        linkLabel: "przechowalniaopon.pl",
        linkDestination: "https://przechowalniaopon.pl/",
      },
    },
...
async function main() {
  if (!fs.existsSync(MANIFEST)) {
    throw new Error(
      `No ${MANIFEST}. Run: node scripts/upload-przechowalniaopon-screenshots.cjs`
    );
  }
