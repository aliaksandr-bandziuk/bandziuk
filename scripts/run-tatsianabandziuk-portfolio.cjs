// scripts/run-tatsianabandziuk-portfolio.cjs
//
// Builds the tatsianabandziuk.com portfolio case study in Sanity: three locale
// documents (en / pl / ru) plus the translation.metadata that ties them
// together, in a single transaction.
//
// Inputs
//   drafts/portfolio-tatsianabandziuk-{EN,PL,RU}.md   the case text, section-tagged
//   drafts/screenshots-tatsianabandziuk/assets.json   image asset ids, written by
//                                                  upload-tatsianabandziuk-screenshots.cjs
//
// Run the screenshot uploader first — this script does not upload images.
//
// Usage:
//   node scripts/run-tatsianabandziuk-portfolio.cjs --dry-run   parse, resolve refs, print, write nothing
//   node scripts/run-tatsianabandziuk-portfolio.cjs             create the documents
...
const BASE_ID = "portfolio-tatsianabandziuk";

const DRAFTS = path.resolve(__dirname, "../drafts");
const SHOTS = path.join(DRAFTS, "screenshots-tatsianabandziuk");
const MANIFEST = path.join(SHOTS, "assets.json");

// The image used as previewImage (a device mockup of the home page); it is kept
...
const SLIDER_FILES = [
  ...
  "9-russian-version.png",
];
// Slider file -> numbered entry in the [SCREENSHOTS] block of the drafts.
const SLIDER_ENTRY = Object.fromEntries(SLIDER_FILES.map((f) => [f, parseInt(f, 10)]));
...
const TECHNOLOGIES = [
  ...
  { canonical: "Web Accessibility", aliases: ["Web Accessibility", "Web Dostępność", "Web Доступность"] },
];

// Internal links are written in the drafts themselves as markdown links,
// [anchor](/href), and converted to link annotations by markdownLinks() below.
const LINK_PLAN = { en: [], pl: [], ru: [] };

// "[anchor](/href)" -> "[[anchor|Mn]]" with Mn resolved through linkMap, so the
// existing span parser turns it into a link mark. Only site-relative links are
// accepted: an external or malformed target in the case text is a mistake.
function markdownLinks(text, linkMap) {
  return text.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_, anchor, href) => {
    if (!href.startsWith("/")) throw new Error(`Only internal links are expected in the case text: ${href}`);
    const id = `M${Object.keys(linkMap).length}`;
    linkMap[id] = href;
    return `[[${anchor}|${id}]]`;
  });
}
...
function readDraft(lang) {
  const p = path.join(DRAFTS, `portfolio-tatsianabandziuk-${lang.toUpperCase()}.md`);
...
function unquote(s) {
  // Polish drafts use „…” quotes, Russian «…», English "…".
...
function buildDoc({ lang, docId, raw, manifest, techIds }) {
  ...
  const mainContent = parseMainContent(raw).map((sec) => {
    ...
    return {
      ...
      _type: "textContent",
      textAlign: "left",
      content: [ptBlock("h3", sec.heading), ...paragraphs.map((t) => ptBlock("normal", markdownLinks(t, linkMap), { linkMap }))],
    };
  });
  ...
  return {
    ...
    keyFeatures: {
      ...
      website: {
        type: "link",
        linkLabel: "tatsianabandziuk.com",
        linkDestination: "https://www.tatsianabandziuk.com/",
      },
    },
...
async function main() {
  if (!fs.existsSync(MANIFEST)) {
    throw new Error(
      `No ${MANIFEST}. Run: node scripts/upload-tatsianabandziuk-screenshots.cjs`
    );
  }
