...
async function main() {
  ...
  const imgInfo = await pg.evaluate(() => {
    const img = document.querySelector('[class*="ctaBand"] img');
    return img ? { src: img.src, alt: img.alt, naturalWidth: img.naturalWidth } : null;
  });
  console.log("CTA band image:", JSON.stringify(imgInfo, null, 1));
  await pg.screenshot({ path: "C:/Users/HP/AppData/Local/Temp/claude/d--applications-bandziuk/fe962a76-7d19-4522-b565-f66ed0f49a5b/scratchpad/cta-schema-fix.png" });
  await browser.close();
}
